...
public class ChunkManager {
     ...
     */
    private Vector2 findBlock(ToolItem tool, Vector2 position, Vector2 direction, Vector2 origin, float maxReach,
                              float xCount, float yCount) {
     ...
     * @return The tile position of a block if found; null if nothing found within reach
     */
    private Vector2 findFarthestAirBlock(Vector2 position, Vector2 direction, Vector2 origin, float maxReach,
                                         float xCount, float yCount, Vector2 farthestAirFound) {
        xCount += direction.x; //Add direction
        yCount += direction.y; //Add direction
        boolean newVal = false; //Flag to see if we switch blocks
        if (xCount >= 1 || xCount <= -1) { //If we went left or right of the last tile
