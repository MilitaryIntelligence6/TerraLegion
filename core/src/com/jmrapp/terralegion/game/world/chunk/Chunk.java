...
import com.jmrapp.terralegion.game.world.block.*;
...
public class Chunk {
    ...
    public void render(OrthoCamera camera, SpriteBatch sb, float centerX, float centerY, int startYDraw, int endYDraw,
                       int startXDraw, int endXDraw) {
                ...
                if (ChunkManager.isOnScreen(x * ChunkManager.TILE_SIZE + (getStartX() * Chunk.CHUNK_SIZE * ChunkManager.TILE_SIZE), y * ChunkManager.TILE_SIZE + (startY * Chunk.CHUNK_SIZE * ChunkManager.TILE_SIZE), camera, centerX, centerY)) {
                    ...
                    if ((type == null || type == BlockType.AIR) || BlockManager.getBlock(type).isTransparent()) {
                        ...
                        if (wallType != null && wallType != BlockType.AIR) {
                            BlockManager.getBlock(wallType).render(camera, sb,
									x * ChunkManager.TILE_SIZE + (startX * Chunk.CHUNK_SIZE * ChunkManager.TILE_SIZE),
									y * ChunkManager.TILE_SIZE + (startY * Chunk.CHUNK_SIZE * ChunkManager.TILE_SIZE),
									getLightValue(x, y));
                        }
                    }
                    if (type != null && type != BlockType.AIR) {
                        BlockManager.getBlock(type).render(camera, sb,
								x * ChunkManager.TILE_SIZE + (startX * Chunk.CHUNK_SIZE * ChunkManager.TILE_SIZE),
								y * ChunkManager.TILE_SIZE + (startY * Chunk.CHUNK_SIZE * ChunkManager.TILE_SIZE),
								getLightValue(x, y));
                    }
    ...
    public Array<LivingEntity> findLivingEntitiesInRange(float x, float y, float range) {
        ...
        for (int j = 1; j < foundEntities.size; j++) {
            ...
            for (i = j - 1; (i >= 0) && (origin.dst(foundEntities.get(i).getX(), foundEntities.get(i).getY()) > dist);
                 i--) {
    ...
    private void calculateSun(int x) {
        ...
        if (lightValue <= .7f) {
            LightUtils.calculateChunkLight(chunkManager, x + (startX * CHUNK_SIZE), highestY + (startY * CHUNK_SIZE),
					1f + BlockManager.getBlock(blocks[x][highestY]).getLightBlockingAmount(), false);
        }
    }
