...
public class Block {
    ...
    public Block(BlockType type, Drawable drawable, float lightBlockingAmount, boolean collides, boolean transparent,
                 float initHealth, float resistance) {
        this.type = type;
    ...
    public void onBreak(ChunkManager chunkManager, Chunk chunk, int chunkTileX, int chunkTileY) {
        if (type != BlockType.AIR) {
            ...
            float lightVal = chunk.getLightValue(chunkTileX, chunkTileY) + lightBlockingAmount; //Get the tile light
            // value
            chunk.setLightValue(LightUtils.MIN_LIGHT_VALUE, chunkTileX, chunkTileY); //set to the smallest so that it
            // gets overwritten
            boolean lightSource =
                    chunk.getBlockProperties(chunkTileX, chunkTileY).hasFlag(BlockProperties.LIT_BY_LIGHT_SOURCE);
            LightUtils.calculateChunkLight(chunkManager, realX, realY, lightVal, lightSource); //update surrounding
            // light values
            float x =
                    (((chunk.getStartX() * Chunk.CHUNK_SIZE) + chunkTileX) * ChunkManager.TILE_SIZE) + MathUtils.random(ChunkManager.TILE_SIZE / 4, ChunkManager.TILE_SIZE / 2);
            float y =
                    (((chunk.getStartY() * Chunk.CHUNK_SIZE) + chunkTileY) * ChunkManager.TILE_SIZE) + (ChunkManager.TILE_SIZE / 2);
    ...
    public boolean onDamage(ChunkManager chunkManager, Chunk chunk, int chunkTileX, int chunkTileY,
                            BlockProperties properties, float toolPower) {
