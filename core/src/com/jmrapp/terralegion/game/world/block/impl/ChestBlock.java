 ...
 * <p>
 * TODO For breaking, the chest could display a menu: "(Break) or (Open)", or it could hold a (Break) button inside
...
public class ChestBlock extends InventoryBlock {
    ...
    public ChestBlock(int inventoryWidth, int inventoryHeight, BlockType type, Drawable drawable,
                      float lightBlockingAmount, boolean collides, boolean transparent, float initHealth,
                      float resistance) {
        super(inventoryWidth, inventoryHeight, type, drawable, lightBlockingAmount, collides, transparent, initHealth
                , resistance);
    }
    ...
    public void onPlace(ChunkManager chunkManager, Chunk chunk, int chunkTileX, int chunkTileY) {
        ...
        this.inventoryScreen = new InventoryScreen(ScreenManager.getCurrent(), chunkManager.getWorld(),
                this.getInventory());
