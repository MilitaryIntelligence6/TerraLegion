...
public class LightUtils {
    ...
    public static void calculateChunkLight(ChunkManager manager, int lightX, int lightY, float lightValue,
                                           boolean lightSource) {
    ...
    private static void applyLight(ChunkManager manager, int currentX, int currentY, float lightValue,
                                   boolean lightSource) {
     ...
     */
    public static void extinguishLight(ChunkManager manager, int lightX, int lightY, float lightValue,
                                       boolean lightSource) {
        if (lightValue > MIN_LIGHT_VALUE) {
            ...
            while (it.hasNext()) {
                ...
                if (currentLight > MIN_LIGHT_VALUE) { //make sure another iteration didnt change the light value
                    // after it was added
                    float lightBlockage =
                            BlockManager.getBlock(light.getChunk().getBlock(light.getX(), light.getY())).getLightBlockingAmount();
                    light.getChunk().setLightValue(MIN_LIGHT_VALUE, light.getX(), light.getY());
                    applyLight(manager, light.getX() + (light.getChunk().getStartX() * Chunk.CHUNK_SIZE),
                            light.getY() + (light.getChunk().getStartY() * Chunk.CHUNK_SIZE),
                            currentLight + lightBlockage, lightSource);
                }
    ...
    public static void extinguishLightAmount(ChunkManager manager, int lightX, int lightY, float originalLightValue,
                                             float subtractedValue, boolean lightSource) {
            ...
            while (it.hasNext()) {
                ...
                if (currentLight > MIN_LIGHT_VALUE) { //make sure another iteration didnt change the light value
                    ...
                    float lightBlockage =
                            BlockManager.getBlock(light.getChunk().getBlock(light.getX(), light.getY())).getLightBlockingAmount();
                    //light.getChunk().setLightValue(MIN_LIGHT_VALUE, light.getX(), light.getY());
                    //applyLight(manager, light.getX() + (light.getChunk().getStartX() * Chunk.CHUNK_SIZE), light
                    // .getY() + (light.getChunk().getStartY() * Chunk.CHUNK_SIZE), currentLight + lightBlockage,
                    // lightSource);
                }
                recycledLights.add(light);
                it.remove();
    ...
    private static void applyExtinguishingLight(ChunkManager manager, int currentX, int currentY, float lightValue,
                                                boolean checkEdges) {
        if (currentX < 0 || currentX >= (ChunkManager.CHUNKS_X * Chunk.CHUNK_SIZE) || currentY < 0 || currentY >= (ChunkManager.CHUNKS_Y * Chunk.CHUNK_SIZE)) {
            return;
        }
        ...
        if (currentLight - lightValue > FLOAT_LOSS_PRECISION) { //If the value of the light is greater than what it
            // should be, then queue it.
            lightQueue.put(posChunk.getStartX() + " " + posChunk.getStartY() + " " + posChunkX + " " + posChunkY,
                    getQueuedLight(posChunk, posChunkX, posChunkY));//Queue this tile to calculate light again
            edgeFlag = true;
        } else if (lightValue - currentLight > FLOAT_LOSS_PRECISION) { //if less than the light it should be then
            // something else will handle it
            return;
        }
        ...
        if (!edgeFlag) {
            ...
            lightQueue.remove(posChunk.getStartX() + " " + posChunk.getStartY() + " " + posChunkX + " " + posChunkY);
            //Remove if in queue
        }

        if (edgeFlag || lightValue <= MIN_LIGHT_VALUE) //Set the checkEdges if we flagged it to do so or if we have
    ...
    private static void applyExtinguishingLightAmount(ChunkManager manager, int currentX, int currentY,
                                                      float lightValue, float subtractedValue, boolean checkEdges) {
        if (currentX < 0 || currentX >= (ChunkManager.CHUNKS_X * Chunk.CHUNK_SIZE) || currentY < 0 || currentY >= (ChunkManager.CHUNKS_Y * Chunk.CHUNK_SIZE)) {
            return;
        }
        Chunk posChunk = manager.getChunkFromTilePos(currentX, currentY);
        ...
        //Assume that the precision loss amount is about .001f
        if (currentLight - lightValue > FLOAT_LOSS_PRECISION || lightValue - currentLight > FLOAT_LOSS_PRECISION) {
            //If the value of the light is greater than what it should be, then queue it.
            //lightQueue.put(posChunk.getStartX() + " " + posChunk.getStartY() + " " + posChunkX + " " + posChunkY,
            // getQueuedLight(posChunk, posChunkX, posChunkY));//Queue this tile to calculate light again
            edgeFlag = true;
        } else if (lightValue - currentLight > FLOAT_LOSS_PRECISION) { //if less than the light it should be then
        ...
        if (!edgeFlag) {
            ...
            posChunk.getBlockProperties(posChunkX, posChunkY).removeFlag(BlockProperties.LIT_BY_LIGHT_SOURCE);
            //lightQueue.remove(posChunk.getStartX() + " " + posChunk.getStartY() + " " + posChunkX + " " +
            // posChunkY);//Remove if in queue
            //}
        }
