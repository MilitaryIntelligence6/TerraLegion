...
public class InventoryScreen implements Screen {
    ...
    private PictureButton craftingBtn = new PictureButton(ResourceManager.getInstance().getDrawable(
            "unselectedCraftingBtn"), 30, Settings.getHeight() - 61 + 10);
    private PictureButton invBtn = new PictureButton(ResourceManager.getInstance().getDrawable("selectedInvBtn"),
            (Settings.getWidth() / 2) - 25, Settings.getHeight() - 61 + 10);
    private PictureButton dropBtn = new PictureButton(ResourceManager.getInstance().getDrawable("dropItemBtn"),
            Settings.getWidth() - 175, 30);
    private PictureButton splitBtn = new PictureButton(ResourceManager.getInstance().getDrawable("splitItemBtn"),
            Settings.getWidth() - 175, 100);
        ...
        if (Gdx.input.justTouched()) {
            ...
            if (cancelBtn.isPressed(touchX, touchY)) {
                ...
                if (selectedInventoryBox != null) {
                    ...
                    if (stack != null) {
                        Drop drop = DropManager.getInstance().getDrop(stack.getItem().getTypeId(), stack.getStack(),
                                world.getPlayer().getX(), world.getPlayer().getY() + world.getPlayer().getHeight());
                        drop.setPickupWaitTime(3f);
                        drop.setVelocity(0, 2);
                        inventory.setItemStack(null, selectedInventoryBox.getInventoryX(),
                                selectedInventoryBox.getInventoryY());
                    ...
                    if (selectedInventoryBox.getItemStack() != null) {
                        if (!inventory.isFull()) {
                            ...
                            if (itemStack.getStack() > 1) {
                                ...
                                inventory.setItemStack(itemStack, selectedInventoryBox.getInventoryX(),
                                        selectedInventoryBox.getInventoryY());
