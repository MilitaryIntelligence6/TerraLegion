        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (inventory[x][y] != null && inventory[x][y].getItem().getTypeId() == item.getTypeId()) {
                    ...
                    wasChanged = true;
                    if (remainder > 0) { //If there is more to be removed then it means the current itemstack is a
						// stack size of 0
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (inventory[x][y] != null && inventory[x][y].getItem().getTypeId() == itemStack.getItem().getTypeId()) {
                    ItemStack stack = inventory[x][y]; //Get the itemstack in its place
                    wasChanged = true;
                    if (stack.removeStack(itemStack).getStack() > 0) { //If there is more to be removed then it means
						// the current itemstack in the inventory is a stack size of 0
                        if (stack.getStack() == 0) { //Double check it is 0
                            stack.destroy(); //Destroy it
                            inventory[x][y] = null; //Set it to null
                            changeFlag = true;
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (inventory[x][y] != null) { //If it is not empty
                    if (inventory[x][y].getItem().getTypeId() == itemStack.getItem().getTypeId()) { //if it is the
						// same item
                        ...
                        if (itemStack.getStack() != count) {
                            if (itemStack.getStack() > 0) { //There was a remainder and less than the original so add
								// it to the inventory
            ...
            for (int x = 0; x < width; x++) {
                if (inventory[x][y] != null) { //If it is not empty
                    if (inventory[x][y].getItem().getTypeId() == item.getTypeId()) { //if it is the same item
                        ...
                        remainder = inventory[x][y].addStack(count); //Merge. Will set the remainder to the current
						// stack
                        if (remainder != count) {
                            if (remainder > 0) { //There was a remainder and less than the original so add it to the
								// inventory
