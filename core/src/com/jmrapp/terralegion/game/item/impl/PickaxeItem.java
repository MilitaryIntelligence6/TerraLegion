...
public class PickaxeItem extends ToolItem {
    ...
    public PickaxeItem(ItemType type, String name, Drawable icon, int maxItemStack, float damage, float power,
    ...
    public boolean onUse(World world, float touchX, float touchY) {
        ...
        if (this.canDamageBlock(type)) {
            if (world.getPlayer().canUseItem(this) && world.getChunkManager().damageBlock(touchX, touchY,
                    this.getPower())) {
