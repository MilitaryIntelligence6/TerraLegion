...
public class PhysicsWorld {
    ...
    public void focusCamera(WorldBody obj, int screenWidth, int screenHeight, int worldWidth, int worldHeight,
                            OrthoCamera camera) {
        float camX = 0, camY = 0;
    ...
    public void checkCollisionsOnBlocks(WorldBody body) {
        ...
        int tileCountVertical = (int) (body.getHeight() / ChunkManager.TILE_SIZE); //Get how many tiles going
        // vertically we need to check to fit the size of the body (height)
        int tileCountHorizontal = (int) (body.getHeight() / ChunkManager.TILE_SIZE); //Get how many tiles going
        // horizontally we need to check to fit the size of the body (width)
        ...
        for (int i = -2; i < tileCountHorizontal + 2; i++) {
            for (int j = -2; j < tileCountVertical + 2; j++) {
                ...
                if (type != null && BlockManager.getBlock(type).collides()) { //If it collides
                    blockBody.set(cx * ChunkManager.TILE_SIZE, cy * ChunkManager.TILE_SIZE, ChunkManager.TILE_SIZE,
                            ChunkManager.TILE_SIZE, BodyType.STATIC);
